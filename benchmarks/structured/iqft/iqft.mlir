...
//
// Hand-written counterpart of iqft.qasm, with 7 result bits. A single qubit is
// measured and reset once per iteration.
//
  ...
  func.func @main() -> memref<7xi1> attributes {passthrough = ["entry_point"]} {
    ...
    %base_end = scf.for %i = %c0 to %size step %c1
      ...
      %angle_end = scf.for %j = %c0 to %i step %c1
        ...
        %bit = memref.load %res[%slot] : memref<7xi1>
        scf.if %bit {
      ...
      memref.store %m, %res[%target] : memref<7xi1>
