...
class VerificationResult:
    """The complete verifier result for one module."""
...
class _Verifier:
    ...
    def verify_module(self, module: object) -> VerificationResult:
        ...
        for index, func in enumerate(functions):
            ...
            if kind == "definition":
                ...
                self._error(loc, f"unsupported function kind {kind!r}")
    ...
    def _verify_op_types(
        ...
        if kind == "qubit":
            ...
            self._verify_scf_op(instruction, subkind, inputs, outputs, types, loc)
        elif kind == "func":
    ...
    def _verify_qubit_gate(
        ...
        for pos, input_type in enumerate(inputs[quantum_inputs:]):
            arg_pos = quantum_inputs + pos
            self._expect_type(input_type, ANY_FLOAT, f"{loc}.inputs[{arg_pos}]")
    ...
    def _verify_int_op(
        ...
        if subkind in INT_CONSTANTS:
            expected = TypeRef("int", INT_CONSTANTS[subkind])
            self._expect_signature(loc, inputs, outputs, [], [expected])
        elif subkind in INT_BINARY_SAME:
            expected = [ANY_INT, ANY_INT]
            if self._expect_signature(loc, inputs, outputs, expected, [ANY_INT]):
    ...
    def _verify_int_array_op(
        ...
        if subkind in INT_CONSTANTS:
            ...
            expected = [ANY_INT_ARRAY, INT32, expected_value]
            self._expect_signature(loc, inputs, outputs, expected, [ANY_INT_ARRAY])
        elif subkind == "length":
    ...
    def _verify_float_op(
        ...
        if subkind in FLOAT_CONSTANTS:
            expected = TypeRef("float", FLOAT_CONSTANTS[subkind])
            self._expect_signature(loc, inputs, outputs, [], [expected])
    ...
    def _verify_scf_op(
        ...
        if subkind == "switch":
            ...
            self._verify_while_like_op(
                while_op.condition, while_op.body, inputs, outputs, types, loc
            )
    ...
    def _verify_for_op(
        ...
        self._expect_region_signature(body, types, (inputs[0], *state), state, loc)
    ...
    def _verify_while_like_op(
        ...
        self._expect_region_signature(condition, types, state, (INT1,), loc)
        self._expect_region_signature(body, types, state, state, loc)
    ...
    def _expect_region_signature(
        ...
        source_types = _lookup_types(_indices(region.sources), types)
        target_types = _lookup_types(_indices(region.targets), types)
    ...
    def _check_metadata_indices(self, _value: object, _loc: str) -> None:
        # Metadata values are AnyPointer payloads and outside this first pass.
        return
    ...
    def _expect_signature(
        ...
        ok = True
        ok = self._expect_count(loc, inputs, len(expected_inputs), "inputs") and ok
        ok = self._expect_count(loc, outputs, len(expected_outputs), "outputs") and ok
        if not ok:
            return False
...
def _has_region(region: object) -> bool:
    try:
        ...
        pass
    return bool(len(region.sources) or len(region.targets) or len(region.operations))
